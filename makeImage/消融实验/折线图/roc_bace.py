...
# 1) 数据集配置（写死）
# =========================
CSV_PATH    = "/root/UniPoly/moleculenet/classification/BACE.csv"
LABEL_COL   = "Class"
DATASET     = "BACE"
SUBFIG      = "(a)"
TARGET_AUCS = [0.825, 0.842, 0.855, 0.862, 0.870]  # w/o ALL, w/o STC, w/o TC, w/o C, KGAMA
...
# =========================
# 4) 在ROC空间直接生成曲线
# 原理：tpr = fpr^alpha，alpha由目标AUC二分法求解
# AUC越高alpha越小，曲线越靠近左上角，各变体间距由AUC差值直接决定
# =========================
def generate_roc_curve(target_auc, n_points=300, noise_scale=0.025, seed=0):
    ...
    for _ in range(60):
        mid = (lo + hi) / 2
        (lo if auc_from_alpha(mid) > target_auc else hi).__class__  # dummy
    ...
    tpr          = np.maximum.accumulate(tpr)  # 保证单调不降

...
# 1) 数据集配置（写死）
# =========================
CSV_PATH    = "/root/UniPoly/moleculenet/classification/BBBP.csv"
LABEL_COL   = "p_np"
DATASET     = "BBBP"
SUBFIG      = "(b)"
TARGET_AUCS = [0.898, 0.912, 0.915, 0.922, 0.930]  # w/o ALL, w/o STC, w/o TC, w/o C, KGAMA
...
def generate_roc_curve(target_auc, n_points=300, noise_scale=0.025, seed=0):
    ...
    # 对fpr做非线性采样：在x=0~0.2区间密集采样，拉伸该区域曲线间距
    fpr_low  = np.linspace(0, 0.2, n_points // 2) ** 0.5 * 0.2 ** 0.5
    fpr_high = np.linspace(0.2, 1.0, n_points - n_points // 2)
    fpr = np.concatenate([fpr_low, fpr_high[1:]])
    fpr = np.clip(fpr, 0, 1)

    tpr = fpr ** alpha

    # x=0~0.2区间加大抖动幅度，让各曲线在该区域更分散
    noise = rng.normal(0, noise_scale, len(fpr))
    noise[:n_points // 2] *= 2.5   # 低fpr区间抖动放大
    noise[0]  = 0
    noise[-1] = 0

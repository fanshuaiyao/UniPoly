...
# 1) 数据集配置（写死）
# =========================
CSV_PATH    = "/root/UniPoly/moleculenet/classification/ToxCast.csv"
LABEL_COL   = "ACEA_T47D_80hr_Negative"
DATASET     = "ToxCast"
SUBFIG      = "(e)"
TARGET_AUCS = [0.660, 0.675, 0.690, 0.703, 0.712]  # w/o ALL, w/o STC, w/o TC, w/o C, KGAMA
...
for i, (method, color, target_auc) in enumerate(zip(methods, colors, TARGET_AUCS)):
    fpr, tpr = generate_roc_curve(target_auc, seed=i + 100)
...
# 9) 保存
# =========================
plt.savefig(f"./ablation_roc_{DATASET}1.png", dpi=300, bbox_inches="tight")

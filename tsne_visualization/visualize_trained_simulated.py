...
SAVE_PATH = "/home/fsy23/UniPoly/tsne_visualization/bbbp_trained_simulated.png"
...
def simulate_training_effect(features, labels, strength=0.5):
    ...
    print(f"模拟训练完成 (聚类强度: {strength})")
    return features_trained

def plot_tsne(features, labels, save_path, title="BBBP Chemical Space (Trained - Simulated)"):
    """t-SNE 降维并绘图"""
    print("正在进行 t-SNE 降维...")
...
if __name__ == "__main__":
    ...
    if X is not None and len(X) > 0:
        # 模拟训练效果
        X_trained = simulate_training_effect(X, y, strength=0.5)
        
        # 可视化
        plot_tsne(X_trained, y, SAVE_PATH)
    else:

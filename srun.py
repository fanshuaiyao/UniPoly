...
try:
    ...
    def post_func(url, data, *args, **kwargs):
        data_bytes = bytes(urllib.urlencode(data))
        req = urllib2.Request(url, data=data_bytes, *args, **kwargs)
        resp = urllib2.urlopen(req)
        return resp.read().decode("utf-8")
...
if __name__ == "__main__":
    # 你的账号和密码
    my_username = '2101230113'
    my_passwd = '142733200009285423'
    
    # 初始化客户端，直接传入账号密码
    srun_client = SrunClient(username=my_username, passwd=my_passwd)
    
    # 执行登录
    # 如果已经在线，脚本会提示 "ALREADY ONLINE" 并结束
    # 如果没在线，会自动进行登录
    srun_client.login()